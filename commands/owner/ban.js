...
async function banCommand(sock, chatId, message) {
    ...
    try {
        // Add user to banned list. Create the data file on first use.
        const dataDir = require('path').join(process.cwd(), 'data');
        const bannedPath = require('path').join(dataDir, 'banned.json');
...
module.exports = {
    ...
    async handler(sock, message, args, context = {}) {
        const chatId = context.chatId || context.from || message.key.remoteJid;
