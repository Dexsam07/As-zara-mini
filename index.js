...
function scheduleReconnect(delayMs) {
  ...
  reconnectTimer = setTimeout(() => {
    reconnectTimer = null;
    startBot().catch(error => console.error('Reconnect failed:', error.message || error));
  }, delayMs);
}

async function startBot() {
  const sessionFolder = path.resolve(`./${config.sessionName}`);

  // Restore the environment session only on first boot. On reconnect,
  // preserve the locally updated Signal keys instead of overwriting them with
  // a stale copy from SESSION_ID.
  const localCredsPath = path.join(sessionFolder, 'creds.json');
  let pairingFallback = !fs.existsSync(localCredsPath);
  if (config.sessionID && !fs.existsSync(localCredsPath)) {
    try {
      const result = restoreSessionId(config.sessionID, sessionFolder);
      if (result.restored) {
        console.log(`📡 Session: restored ${result.format} auth (${result.files} file${result.files === 1 ? '' : 's'})`);
        pairingFallback = false;
        if (!result.complete) {
          console.warn('📡 Session: legacy credentials-only format detected; generate a DEX~ session for reliable reconnects.');
        }
      }
    } catch (error) {
      console.error('📡 Session: invalid or corrupt session ID:', error.message);
      console.warn('📡 Session: switching to phone-number pairing mode.');
      pairingFallback = true;
  ...
  // Pairing-code fallback: never print a QR. The number must contain only
  // digits with country code, for example 919876543210.
  let pairingRequested = false;
  const requestPairingIfNeeded = async () => {
    if (!pairingFallback || pairingRequested || state.creds.registered) return;
    const phoneNumber = String(config.pairingNumber || '').replace(/\D/g, '');
    if (!phoneNumber) {
      console.error('📱 Pairing required: set pairingNumber in config.js or PAIRING_NUMBER in the environment.');
      return;
    }
    if (phoneNumber.length < 8 || phoneNumber.length > 15) {
      console.error('📱 Pairing number is invalid. Use country code plus digits only.');
      return;
    }
  ...
  if (pairingFallback) setTimeout(() => requestPairingIfNeeded(), 1500);
  ...
  const watchdogInterval = setInterval(async () => {
    if (Date.now() - lastActivity > INACTIVITY_TIMEOUT && sock.ws?.readyState === 1) { // WebSocket open but inactive
      ...
      scheduleReconnect(5000); // Slightly longer delay
    }
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (qr) {
      console.log('📱 QR event ignored: pairing-code mode is enabled.');
...
process.on('uncaughtException', (err) => {
  ...
  // Let the process supervisor restart the bot instead of continuing in an
  // unknown state after a fatal exception.
  shuttingDown = true;
  process.exitCode = 1;
